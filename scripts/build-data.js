 ...
 *   2. Census ACS 5-Year (2023)       → Demographics (population, income, poverty)
 *   3. Census TIGERweb                → Land area & county interior points
 *
 * Usage:  node scripts/build-data.js
 *
 * Environment:
 *   CENSUS_API_KEY  (recommended) — Census API key. The Census API blocks
 *                   keyless requests from shared/cloud IPs, so this is
 *                   effectively required when running in CI. Free signup:
...
const ARCGIS_BDC_URL =
    ...
    'FCC_Broadband_Data_Collection_June_2023_with_Provider_Counts/FeatureServer/1/query';
...
function fetchJSON(url) {
    // -L follows redirects (Census redirects keyless/quota requests to an
    // HTML "Missing Key" page; without -L curl returns an empty body).
    const result = execSync(`curl -sL "${url}"`, {
    ...
    try {
        ...
        if (/Missing Key|valid[^.]*key/i.test(text)) {
            throw new Error(
                'Census API rejected the request (missing/invalid key). Set the ' +
                'CENSUS_API_KEY env var — free signup at ' +
                'https://api.census.gov/data/key_signup.html. Response: ' + snippet
            );
        }
        throw new Error('Expected JSON but got non-JSON response. Body: ' + snippet);
    }
}
...
function fetchCensusDemographics() {
    ...
    const url = `${CENSUS_ACS_URL}?get=${vars}&for=county:*${keyParam}`;
    const data = fetchJSON(url);
...
function writeDataJS(counties) {
    ...
    lines.push(` *   FCC Broadband Data Collection (BDC) — June 2023 release via ArcGIS Living Atlas`);
    lines.push(` *   Census ACS 5-Year Estimates (2023)`);
